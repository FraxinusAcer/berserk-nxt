/*
SELECT GROUP_CONCAT(card SEPARATOR ',') FROM (SELECT CAST(SUBSTRING(card, 1, LOCATE(' ', card) - 1) AS UNSIGNED) AS card, AVG(pick) AS avg_pick FROM `draft_picks` WHERE game = 'restart' AND `set` = 5 AND dt >= '2024-09-01' GROUP BY card ORDER BY 2 DESC) t1;
*/
