/*
SELECT GROUP_CONCAT(card SEPARATOR ',') FROM (SELECT CAST(SUBSTRING(card, 1, LOCATE(' ', card) - 1) AS UNSIGNED) AS card, AVG(pick) AS avg_pick FROM `draft_picks` WHERE game = 'restart' AND `set` = 4 AND dt >= '2024-09-01' GROUP BY card ORDER BY 2 DESC) t1;
*/
...
export const motd_order = {
  ...
  "50": [101,175,62,213,30,28,84,170,88,104,64,219,69,35,68,3,33,102,36,217,107,198,192,53,197,19,103,128,118,20,51,216,56,160,108,127,122,70,177,180,55,161,11,140,201,158,96,209,178,135,142,169,139,90,39,137,67,34,92,129,200,220,52,126,159,214,23,77,105,134,8,176,123,172,58,189,163,162,73,49,41,27,1,202,13,14,47,210,148,16,167,157,29,93,114,207,191,144,196,125,10,72,99,121,80,60,110,156,50,131,21,205,188,143,165,145,87,185,106,82,59,100,181,48,133,6,190,15,26,24,65,94,46,66,117,132,7,195,18,37,63,97,150,5,141,136,174,81,164,78,71,98,86,155,120,91,206,43,32,76,154,31,153,112,38,130,138,45,17,193,74,95,115,151,4,83,166,183,57,212,152,171,215,54,184,2,168,179,89,111,211,146,22,116,194,182,208,119,85,204,113,124,199,218,173,42,61,9,75,40,25,187,203,149,79,186,12,147,44,109]
}
...
function getBoosterV2(cardsByRarityAndColor) {
  ...
  const booster = [];
  booster.push(...((Math.random() < 4 / 24 && Object.keys(cardsByRarityAndColor[4]).length > 0) ? selectRandomCardsFromDifferentColors(cardsByRarityAndColor[4], 1) : selectRandomCardsFromDifferentColors(cardsByRarityAndColor[3], 1)));
...
export async function doAIPick(current_deck, booster, timeout = 2000) {
  const controller = new AbortController();
  const timeoutId = setTimeout(() => controller.abort(), timeout);
  try {
    const response = await fetch('http://127.0.0.1:21285/predict', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ 'context': current_deck.filter((x) => x), 'options': booster.map((card) => card['id']) }),
